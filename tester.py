...
import requests, sys, json, csv, os, urllib, smtplib
...
class WebsiteTags:
    ...
    def scrapeMicrodata(self, sitename, prefType):
        ...
        endfile = open("temp/%s/tagsFound.json" % sitename, "wb")
        json.dump(self.foundTags, endfile, indent=4, sort_keys=True)
        endfile.close()
        endfile2 = open("temp/%s/tagsGuide.json" % sitename, "wb")
    ...
    def validateWebsiteMicrodata(self, sitename):
        ...
        found_types = open("temp/%s/typesFound.txt" % sitename, "rb")    # All the types found in the website
        f = open("temp/%s/tagsFound.json" % sitename, "rb")
        r = open("bioschemas/bioschemasTags.json", "rb")
        found = json.load(f)
        ref = json.load(r)

        report = open("temp/%s/report.csv" % sitename, "wb")
        ...
        for line in found_types:
            ...
            miss = open("temp/%s/missingMin.csv" % sitename, "ab")  # Required props missing in the website
            w_miss = csv.writer(miss)

            prov = open("temp/%s/providedRec.csv" % sitename, "ab")  # Not required props provided in the website
            ...
            if line == "Event" or line == "Organization" or line == "Person" or line == "Training" or line == "CreativeWork":
                ...
                miss = open("temp/%s/missingMin.csv" % sitename, "rb")
                miss_r = csv.reader(miss)

                prov = open("temp/%s/providedRec.csv" % sitename, "rb")
...
class UpdateRegistry:
    ...
    def createChartFile(self, website, type_bs):
        ...
        for typ in type_bs:
            ...
            with open("temp/%s/tagsGuide.json" % website, "rb") as f, open("temp/%s/tagsFound.json" % website, "rb") as g:
                data = json.load(f)
                data2 = json.load(g)
                ...
                endfile = open("temp/%s/chartData_%s.csv" % (website, typ), "wb")
                w = csv.writer(endfile)
                w.writerow(["label", "score", "weight", "width", "color", "mean"])

                maxFile = open("temp/%s/maxData_%s.csv" % (website, typ), "wb")
...
class NotifMail:
    """Send a notification email when the scraping is done."""
    ...
    def mail_alert(self):
        ...
        try:
            fromaddr = "bioschemas.registry@gmail.com"
            toaddr = self.recipient
            msg = """Thanks for using Bioschemas Registry & Validator.
            The website you submitted has just been tested, and can be accessed from the Registry page."""
            # Insert link for the Registry page
            username = "bioschemas.registry"
            password = ""
